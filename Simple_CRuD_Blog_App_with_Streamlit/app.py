...
def main():
    ...
    if choice == "Home":
        ...
        for i in result:
            ...
            st.write(title_temp.format(
                i[1], i[0], short_article), unsafe_allow_html=True)
        ...
        for i in post_result:
            ...
            st.markdown(head_message_temp.format(
                i[1], i[0], i[3]), unsafe_allow_html=True)
        ...
        if st.button('Search'):
            ...
            for i in article_result:
                st.text("Reading Time:{} minutes".format(
                    readingTime(str(i[2]))))
                # st.write(article_temp.format(i[1],i[0],i[3],i[2]),unsafe_allow_html=True)
                st.write(head_message_temp.format(
                    i[1], i[0], i[3]), unsafe_allow_html=True)
                st.write(full_message_temp.format(
                    i[2]), unsafe_allow_html=True)
        ...
        if st.checkbox("BarH Plot"):
            ...
            new_df = clean_db
            new_df['Length'] = new_df['Article'].str.len()
            barh_plot = new_df.plot.barh(
                x='Author', y='Length', figsize=(10, 10))
            st.write(barh_plot)
            st.pyplot()
